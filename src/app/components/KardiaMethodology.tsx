...
export function KardiaMethodology({ onPhaseHover }: KardiaMethodologyProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center py-20 px-4 relative bg-black overflow-hidden">
      ...
      {/* 1. Central Kardia Circle (Animated by state) */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none z-10">
        ...
        >
          {/* Pulsing Rings logic remains same... */}
          <motion.div 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl w-full mt-8 relative z-20">
        {phases.map((phase, index) => (
          <motion.div
            ...
            // TOUCH EVENTS (Mobile)
            onTouchStart={() => handlePhaseInteraction(phase.id)}
            onTouchEnd={() => handlePhaseInteraction(null)}
            // IMPORTANT: 'touch-pan-y' allows vertical scrolling while touching cards
            className="relative group cursor-pointer touch-pan-y" 
          ...
          >
            <motion.div 
              ...
              animate={{
                ...
                y: activePhase === phase.id ? -10 : 0
              }}
            >
              {/* Card Internals (Icon, Title, Copy) */}
              ...
              {/* Progress Bar that fills on interaction */}
