...
// ⚠️ Ensure this matches your DEPLOYED Web App URL from Google Apps Script
...
export function ImpactSidebar({ isOpen, onClose, items, onRemove }: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      /**
       * ⚡️ DATA ALIGNMENT
       * We structure the payload to match exactly what your Apps Script 
       * JSON.parse(e.postData.contents) logic is looking for.
       */
      const payload = {
        userInfo: {
      ...
      console.error("Transmission_Failure:", e);
      setStatus('error');
          ...
          >
            ...
            <div className="p-8 md:p-12 border-b border-white/5 flex justify-between items-center bg-gradient-to-r from-purple-900/10 to-transparent">
              <div>
                ...
                <p className="font-mono text-[9px] uppercase tracking-[0.3em] text-white/40 mt-1">
                  Status: {status === 'loading' ? 'Transmitting...' : 'Ready_for_Transmission'}
                </p>
              </div>
              <button 
                onClick={onClose} 
                className="hover:rotate-90 transition-transform p-2 bg-white/5 rounded-full text-white"
              >
                <X size={20}/>
              </button>
            </div>

            {/* Main Content / Selection List */}
                      ...
                      {items.map((i: any) => (
                        ...
                        >
                          ...
                          <button 
                            ...
                            className="opacity-0 group-hover:opacity-100 hover:text-red-500 transition-all p-2"
                          >
                            <Trash2 size={14}/>
                          </button>
                        </motion.div>
            ...
            <div className="p-8 md:p-12 bg-white/[0.02] border-t border-white/10 space-y-6">
              {items.length > 0 && (
                <div className="space-y-4">
                   <div className="flex items-center gap-3 text-[10px] font-mono uppercase text-white/40">
                      <ShieldCheck size={14} className={cn(status === 'success' ? "text-green-500" : "text-purple-500")} />
                  ...
                  <div className="relative group">
                    <input 
                      ...
                      className="w-full bg-black border border-white/10 p-5 font-mono text-xs outline-none focus:border-purple-500 transition-all text-white placeholder:text-white/20 disabled:opacity-50" 
                    />
              ...
              <button 
                ...
                disabled={status === 'loading' || status === 'success' || items.length === 0}
                className={cn(
              ...
              >
                {status === 'loading' ? (
                  <span className="flex items-center gap-2">
                    <motion.span animate={{ rotate: 360 }} transition={{ repeat: Infinity, ease: "linear" }}>
                      <Activity size={16}/>
                    </motion.span>
                    Transmitting_Data...
                  </span>
              ...
              {items.length > 0 && (
                <p className="text-center font-mono text-[8px] uppercase text-white/20 tracking-tighter">
                  System Notice: Initializing custom digital architecture audit upon transmission.
                </p>
              )}
            </div>
          </motion.div>
