...
export function HeroSection({ isDark }: HeroProps) {
  ...
  useEffect(() => {
    ...
    if (savedTitle) setDisplayTitle(savedTitle);
  }, []);
      ...
      <motion.div style={{ y: y1, opacity }} className="relative z-10 text-center w-full max-w-[100vw]">
        <div className="flex justify-center gap-6 md:gap-12 mb-12">
          {["System_Active", "Region_IN", "Narrative_Engaged"].map((stat) => (
            <motion.span key={stat} initial={{ opacity: 0 }} animate={{ opacity: 0.4 }} className="font-mono text-[8px] md:text-[9px] uppercase tracking-[0.3em]">{stat}</motion.span>
          ))}
        </div>
        ...
        <div className="relative group">
          ...
          >
            {displayTitle}
            <motion.div animate={{ left: ['-10%', '110%'] }} transition={{ duration: 2.5, repeat: Infinity, ease: "linear" }}
