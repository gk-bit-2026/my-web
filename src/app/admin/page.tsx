...
import { 
  ShieldCheck, Layout, Eye, Save, Download, Upload, AlertTriangle, MapPin, Tablet
} from 'lucide-react';
...
export default function AdminPortal() {
  ...
  const handleLogin = () => {
    // HOTWIRE: Use '000000' for first-time setup
  ...
  if (!isAuth) return (
    <div className="h-screen bg-black flex items-center justify-center font-mono text-white">
      <div className="w-full max-w-sm border border-purple-500/20 p-8 bg-zinc-900/10 backdrop-blur-md">
        ...
        />
        <button onClick={handleLogin} className="w-full py-3 bg-purple-600 text-[10px] font-bold hover:bg-purple-500">INITIALIZE</button>
      </div>
    </div>
  );
          ...
          <div className="space-y-12">
            <div className="grid grid-cols-2 gap-8">
              <div className="p-6 border border-white/5 bg-white/[0.02]">
                <h3 className="text-[9px] opacity-40 mb-4 tracking-widest">PERSISTENCE</h3>
                <div className="flex gap-4">
                  <button onClick={exportData} className="flex items-center gap-2 bg-white text-black px-4 py-2 text-[10px] font-black"><Download size={14}/> EXPORT_JSON</button>
                  ...
                  <input type="file" ref={fileInputRef} className="hidden" accept=".json" onChange={(e:any) => {
                    ...
                    reader.onload = (ev) => { updateDb(JSON.parse(ev.target?.result as string)); alert("RELOADED"); };
              ...
              <div className="p-6 border border-white/5 bg-white/[0.02]">
                ...
                {showQR && (
                  <div className="mt-4 p-2 bg-white inline-block">
                    <QRCodeSVG value={`otpauth://totp/GK?secret=${db.auth.secret}&issuer=Graphikardia`} size={100} />
                  </div>
                )}
              </div>
            </div>
            <div className="p-4 bg-red-500/5 border border-red-500/20 text-red-500 text-[9px] flex items-center gap-4">
              <AlertTriangle size={14} /> WARNING: SYSTEM STATE IS LOCAL. DUMP JSON TO SOURCE CODE BEFORE DEPLOYMENT.
            </div>
          </div>
        )}
      </main>
